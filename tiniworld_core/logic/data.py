...
from prophet.plot import performance_metrics, plot_yearly
...
#
    ...
    def get_stores_ds_alltime(self) -> dict:
        ...
        Its keys are 'store_code'
        Its values is pandas.DataFrames loaded from csv files.
    ...
    def get_ratio(self,store=None):
        ...
        # remove all none playground locations
        y = list(df.index)
        l = [(i[:2])=='TW' for i in y]
        df = df[l]
...
#
    def load_model(self,store_name):
        ...
        model_path = os.path.join(os.path.expanduser(LOCAL_MODEL_PATH), f"{store_name}_prophet_model.json")
...
#
    def cv_model(self,df,location) -> pd.DataFrame:
        ...
        Fits and trains a model with this params and saves it to a given folder (LOCAL_MODEL_PATH)
        ...
        save_path = os.path.join(os.path.expanduser(LOCAL_MODEL_PATH),"")
